...
public class Screen10Controller implements Initializable, ControlledScreen {
    ...
    private void updateData(ActionEvent event) {
        ...
        if(!firstname.getText().equals("") && !lastname.getText().equals("") && !username.getText().equals("") && !password.getText().equals("") && !course.getText().equals("") && !selectlecture.getValue().equals("")){
            try {
                String query = ("UPDATE students(firstname,lastname,username,course,lecture,password) set('"+FirstName+"','"+LastName+"','"+UserName+"','"+Course+"','"+Lecture+"','"+PassWord+"')where studentid ='" );
                connection.st.executeUpdate(query);
                System.out.println("Update succussful");
            } catch (Exception e) {
                System.out.println("Error "+e);
